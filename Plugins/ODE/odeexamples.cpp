...
int ODEExamples::realPendulumFunction(double t, const double y[], double f[], void *params)
{
    (void)(t);
    const double r = ((double*)params)[0];
    const double w = ((double*)params)[1];
    ...
    f[0] = y[1];
    f[1] = -r * y[1] - w2 * std::sin(y[0]);

    return GSL_SUCCESS;
}

int ODEExamples::realPendulumJacobian(double t, const double y[], double *dfdy, double dfdt[], void *params)
{
    (void)(t);
    const double r = ((double*)params)[0];
    const double w = ((double*)params)[1];
    ...
    gsl_matrix_set(m, 0, 0, 0.0);
    gsl_matrix_set(m, 0, 1, 1.0);
    gsl_matrix_set(m, 1, 0, -w2 * cos(y[0]));
    gsl_matrix_set(m, 1, 1, -r);
    dfdt[0] = 0.0;
    dfdt[1] = 0.0;
